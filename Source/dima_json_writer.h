...
extern "C" {
	...
	DIMA_JSON_WRITER_DEF void JSONAddF32(json_writer* Writer, char* Key, float Value);
...
void JSONAddF32(json_writer* Writer, char* Key, float Value) {
	char LineBuf[256];
	DIMA_JSON_WRITER_SPRINTF(LineBuf, "\"%s\": %f", Key, Value);
